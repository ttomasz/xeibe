...
| `EPSG-v13_103-PostgreSQL.zip`, `EPSG-v13_103-WKT.Zip`, `EPSG-13_103-SHP.zip` | EPSG Geodetic Parameter Dataset v13.103, <https://epsg.org/download-dataset.html> (free registration required; `scripts/fetch_epsg_dataset.py` signs in and downloads) | 2026-09-20 (by user) | EPSG Dataset Terms of Use, <https://epsg.org/terms-of-use.html> — **not** an open licence: IOGP ownership must be acknowledged and the terms passed on | Input to `scripts/gen_crs_tables.py`. The PostgreSQL scripts build `crates/xeibe-crs`; the WKT release is the differential check. See `crates/xeibe-crs/EPSG-NOTICE.md` |
